...
class RubleHistoryAdapter(context: Context) :
    RecyclerView.Adapter<RubleHistoryAdapter.ViewHolder>() {
    val context = context
    private val dataSet: MutableList<RubleAccDbModel> = mutableListOf()
    fun addData(data: MutableList<RubleAccDbModel>) {
        dataSet.clear()
        dataSet.addAll(data)
    ...
    override fun onBindViewHolder(viewHolder: ViewHolder, position: Int) {
        ...
        if (item.operationType=="PLUS"){
            ...
            viewHolder.value.text = "+ "+item.rubleValue.formatValue()+" ₽"
